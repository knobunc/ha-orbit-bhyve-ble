...
class DeviceState:
    ...
    water_volume_gal: float | None = None
...
class BHyveBleDeviceBase(abc.ABC):
    ...
    def _observe_plaintext(self, pt: bytes) -> None:
        """Parse device data from decrypted BLE notification payloads.
        ...
        type_byte = pt[2]
        seq = pt[3]
        is_reply = bool(type_byte & 0x40)

        if is_reply and seq == 0x03 and len(pt) >= 12:
            self._parse_battery(pt)
        elif is_reply:
            _LOGGER.debug(
                "%s: unhandled notif type=0x%02x seq=0x%02x len=%d pt=%s",
                self.mac, type_byte, seq, len(pt), pt.hex(),
            )

    def _parse_battery(self, pt: bytes) -> None:
        """Extract battery mV from info-ack (seq=0x03) response."""
    ...
    async def refresh_state(self) -> DeviceState:
        """Default: refresh BLE-connection liveness and RSSI. Subclasses can
        extend with a status-request roundtrip."""
        if self.connection is not None:
